    ...
    const fetchPredictions = async () => {
      ...
      try {
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (details.status === "0" || details.status === 0) {
        ...
        {loading ? (
          ...
          <Grid container spacing={3}>
            {predictions.length === 0 && (
              <Grid item xs={12}><Typography>No active predictions.</Typography></Grid>
            )}
              ...
              <Grid item xs={12} sm={6} md={4} key={pred.id}>
                <Card elevation={3} sx={{ borderRadius: 3 }}>
                  <CardContent>
                    ...
                    <Typography color="text.secondary">Ends: {new Date(Number(pred.endTime) * 1000).toLocaleString()}</Typography>
